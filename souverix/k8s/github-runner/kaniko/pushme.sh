...
# pushme.sh - Push Kaniko GitHub Runner container to registry with SemVer versioning
...
if [[ -f /home/dasm/gh_token ]]; then
    GITHUB_TOKEN=$(cat /home/dasm/gh_token)
elif [[ -z "${GITHUB_TOKEN:-}" ]]; then
    echo "Error: GITHUB_TOKEN environment variable required or /home/dasm/gh_token file" >&2
...
if [[ "${local_tag}" != "latest" ]]; then
    ${RUNTIME} tag "${LOCAL_IMAGE}" "${TAG_IMAGE}"
fi

echo ":key: Logging in to ${repo}..."
echo "${GITHUB_TOKEN}" | ${RUNTIME} login "${repo}" \
    --username "${repo#ghcr.io/}" \
    --password-stdin
...
if [[ "${local_tag}" != "latest" ]]; then
    echo ":arrow_up: Pushing ${TAG_IMAGE}..."
    ${RUNTIME} push "${TAG_IMAGE}"
fi
